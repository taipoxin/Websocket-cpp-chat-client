...
FileLogger WsController::l = FileLogger(Config::logFileName);
...
void WsController::initWs() {
	ws = initWebSocket();
}

// dynamic -> string
vector<Entities::MessageEntity> WsController::listDynamicToMessageEntities(vector<boost::any> ll) {
	vector<Entities::MessageEntity> entities;
	/*
	for(boost::any const& m : ll)
	{
		auto objCast = boost::any_cast<Entities::MessageEntity>(m);
		auto mess = Entities::MessageEntity();
		mess.from = objCast.from;
		mess.message = objCast.message;
		mess.time = objCast.time;
		entities.push_back(mess);
	}
	*/
	return entities;
}

void WsController::tryToConnect(WebSocket ws) {
	/*
	while (!ws.IsAlive)
	{
	...
	*/
	this->l.log("ws connected");
}

	
WebSocket WsController::getWs() {
	return ws;
}

void WsController::setSigninWindow(Signin w) {
	signinWindow = w;
}
void WsController::setSignupWindow(Signup w) {
	signupWindow = w;
}

void WsController::setMainWindow(MainWindow w) {
	mainWindow = w;
}


WebSocket WsController::initWebSocket() {
	return this->ws;
	/*
